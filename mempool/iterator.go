...
func (i *EVMMempoolIterator) Next() mempool.Iterator {
	...
	i.advanceCurrentIterator(nextEVMTx, nextCosmosTx)
...
func (i *EVMMempoolIterator) advanceCurrentIterator(nextEVMTx *txpool.LazyTransaction, nextCosmosTx sdk.Tx) {
	useEVM := i.shouldUseEVM()

	if useEVM && nextEVMTx != nil {
