	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.expError == "" {
				...
				suite.Require().Contains(evmApp.GetErc20Keeper().GetDynamicPrecompiles(evmCtx), tokenPair.Erc20Address)
			} else {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.expError == "" {
				...
				suite.Require().Contains(evmApp.Erc20Keeper.GetDynamicPrecompiles(ctxA), tokenPair.Erc20Address)
			} else {
