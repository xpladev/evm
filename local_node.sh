...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  jq '.app_state.erc20.token_mappings=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"atest",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
