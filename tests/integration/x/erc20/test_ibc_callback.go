	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				...
				_, err = s.network.App.GetEVMKeeper().CallEVM(ctx, contracts.ERC20MinterBurnerDecimalsContract.ABI, s.keyring.GetAddr(0), contractAddr, true, big.NewInt(100), "mint", types.ModuleAddress)
				s.Require().NoError(err)
