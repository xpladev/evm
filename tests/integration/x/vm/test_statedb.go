...
func (s *KeeperTestSuite) TestSelfDestruct() {
...
// 			func(vmdb vm.StateDB) {
// 				for i := 0; i < 5; i++ {
// 					vmdb.SetState(s.Keyring.GetAddr(0), common.BytesToHash([]byte(fmt.Sprintf("key%d", i))), common.BytesToHash([]byte(fmt.Sprintf("value%d", i))))
// 				}
// 			},
...
// 			"filter state",
// 			func(vmdb vm.StateDB) {
// 				vmdb.SetState(s.Keyring.GetAddr(0), common.BytesToHash([]byte("key")), common.BytesToHash([]byte("value")))
// 				vmdb.SetState(s.Keyring.GetAddr(0), common.BytesToHash([]byte("filterkey")), common.BytesToHash([]byte("filtervalue")))
// 			},
// 			func(key, value common.Hash) bool {
...
// 			tc.malleate(vmdb)
//
// 			err := vmdb.ForEachStorage(s.Keyring.GetAddr(0), tc.callback)
// 			s.Require().NoError(err)
// 			s.Require().Equal(len(tc.expValues), len(storage), fmt.Sprintf("Expected values:\n%v\nStorage Values\n%v", tc.expValues, storage))
//
...
// 			s.Require().ElementsMatch(tc.expValues, vals)
// 		})
