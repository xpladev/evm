...
func TestIntegrationTestSuite(t *testing.T, create network.CreateEvmApp, options ...network.ConfigOption) {
	...
	_ = Describe("ERC20 Extension -", func() {
		...
		BeforeEach(func() {
			...
			nativePrecompiles := is.network.App.GetErc20Keeper().GetNativePrecompiles(is.network.GetContext())
			Expect(len(nativePrecompiles)).To(Equal(1))
			Expect(common.HexToAddress(nativePrecompiles[0])).To(Equal(common.HexToAddress(testconstants.WEVMOSContractMainnet)))
			...
			revertContractAddr, err = is.factory.DeployContract(
				...
				testutiltypes.ContractDeploymentData{
					...
					// NOTE: we're passing the precompile address to the constructor because that initiates the contract
					// to make calls to the correct ERC20 precompile.
					ConstructorArgs: []interface{}{common.HexToAddress(nativePrecompiles[0])},
				},
			)
		...
		Context("metadata query -", func() {
			...
			Context("for a token with available metadata", func() {
				...
				BeforeEach(func() {
					...
					is.precompile, err = is.setupNewERC20PrecompileForTokenPair(*is.network, tokenPairs[0])
